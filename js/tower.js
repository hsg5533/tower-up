(function () {
  ...
  function normalizex(x) {
    return Game.Math.normalize(x, 0, tower.w);
  } // wrap x-coord around to stay within tower boundary
  function normalizeColumn(col) {
    return Game.Math.normalize(col, 0, tower.cols);
  } // wrap column  around to stay within tower boundary
  function x2col(x) {
    return Math.floor(normalizex(x) / COL_WIDTH);
  } // convert x-coord to tower column index
  function y2row(y) {
    return Math.floor(y / ROW_HEIGHT);
  } // convert y-coord to tower row    index
  function col2x(col) {
    return col * COL_WIDTH;
  } // convert tower column index to x-coord
  function row2y(row) {
    return row * ROW_HEIGHT;
  } // convert tower row    index to y-coord
  function x2a(x) {
    return 360 * (normalizex(x) / tower.w);
  } // convert x-coord to an angle around the tower
  function tx(x, r) {
    return r * Math.sin((normalizex(x - camera.rx) / tower.w) * 2 * Math.PI);
  } // transform x-coord for rendering
  function ty(y) {
    return HEIGHT - HORIZON - (y - camera.ry);
  } // transform y-coord for rendering
  function nearColCenter(x, col, limit) {
    return limit > Math.abs(x - col2x(col + 0.5)) / (COL_WIDTH / 2);
  } // is x-coord "near" the center  of a tower column
  function nearRowSurface(y, row) {
    return y > row2y(row + 1) - ROW_SURFACE;
  } // is y-coord "near" the surface of a tower row
  ...
  var Tower = Class.create({
    ...
    getCell: function (row, col) {
      if (row < 0) return this.ground;
      else if (row >= this.rows) return this.air;
      else return this.map[row][normalizeColumn(col)];
    },
  ...
  var Player = Class.create({
    ...
    update: function (dt) {
      ...
      if (this.stepping) return this.stepUp();
      else if (this.hurting) return this.hurt(dt);
      ...
      if (this.climbing) {
        this.ddy = 0;
        if (this.input.up) this.dy = this.climbdy;
        else if (this.input.down) this.dy = -this.climbdy;
        else this.dy = 0;
      }

      if (this.input.left) this.ddx = this.ddx - accel;
      else if (wasleft) this.ddx = this.ddx + friction;

      if (this.input.right) this.ddx = this.ddx + accel;
      else if (wasright) this.ddx = this.ddx - friction;

      if (this.input.jump && (!falling || this.fallingJump)) this.performJump();
    ...
    updatePosition: function (dt) {
      ...
      this.dx = Game.Math.bound(
        this.dx + dt * this.ddx,
        -this.maxdx,
        this.maxdx
      );
      this.dy = Game.Math.bound(
        this.dy + dt * this.ddy,
        -this.maxdy,
        this.maxdy
      );
    ...
    animate: function () {
      ...
      else if (this.climbing) Game.animate(FPS, this, PLAYER.BACK);
      ...
      else Game.animate(FPS, this, PLAYER.STAND);
    },
    ...
    checkCollision: function () {
      ...
      if (
        ...
        !ml.blocked &&
        !tl.blocked &&
        nearRowSurface(this.y + bl.y, bl.row)
      )
      ...
      if (
        ...
        !mr.blocked &&
        !tr.blocked &&
        nearRowSurface(this.y + br.y, br.row)
      )
        return this.collideDown(br);

      if (fallingDown && ld.ladder && !lu.ladder) return this.collideDown(ld);
      ...
      if (climbingDown && ld.blocked) return this.stopClimbing(ld);

      if (runningRight && tr.blocked && !tl.blocked) return this.collide(tr);

      if (runningRight && mr.blocked && !ml.blocked) return this.collide(mr);

      if (runningRight && br.blocked && !bl.blocked) {
        if (falling) return this.collide(br);
        else return this.startSteppingUp(DIR.RIGHT);
      }
      ...
      if (runningLeft && bl.blocked && !br.blocked) {
        if (falling) return this.collide(bl, true);
        else return this.startSteppingUp(DIR.LEFT);
      }

      var onLadder =
        (lu.ladder || ld.ladder) && nearColCenter(this.x, lu.col, LADDER_EDGE);

      // check to see if we are now falling or climbing
      if (
        !climbing &&
        onLadder &&
        ((lu.ladder && this.input.up) || (ld.ladder && this.input.down))
      )
      ...
      if (climbing && !onLadder) return this.stopClimbing();
    ...
    updateCollisionPoint: function (point) {
      ...
      if (point.cell.monster) {
        ...
        if (
          Game.Math.between(
            this.x + point.x,
            monster.x + monster.nx,
            monster.x + monster.nx + monster.w
          ) &&
          Game.Math.between(
            this.y + point.y,
            monster.y + monster.ny,
            monster.y + monster.ny + monster.h
          )
        ) {
    ...
    performJump: function () {
      if (this.climbing) this.stopClimbing();
      this.dy = 0;
    ...
    stepUp: function () {
      ...
      if (--this.stepCount == 0) this.stepping = DIR.NONE;
    },
  ...
  var Monsters = Class.create({
    ...
    update: function (dt) {
      var n,
        max,
        all = this.all;
      for (n = 0, max = all.length; n < max; n++) all[n].update(dt);
    },
  ...
  var Monster = Class.create({
    initialize: function (row, col, type) {
      ...
      if (type.vertical) {
        ...
        while (
          this.minrow > 0 &&
          !tower.map[this.minrow - 1][col].platform &&
          !tower.map[this.minrow - 1][col].ladder
        )
          this.minrow--;
        while (
          this.maxrow < tower.rows - 1 &&
          !tower.map[this.maxrow + 1][col].platform &&
          !tower.map[this.maxrow + 1][col].ladder
        )
          this.maxrow++;
      ...
      if (type.horizontal) {
        ...
        this.maxcol = col;
        while (
          this.mincol != normalizeColumn(col + 1) &&
          !tower.getCell(row, this.mincol - 1).platform &&
          !tower.getCell(row, this.mincol - 1).ladder &&
          tower.getCell(row - 1, this.mincol - 1).platform
        )
          this.mincol = normalizeColumn(this.mincol - 1);
        while (
          this.maxcol != normalizeColumn(col - 1) &&
          !tower.getCell(row, this.maxcol + 1).platform &&
          !tower.getCell(row, this.maxcol + 1).ladder &&
          tower.getCell(row - 1, this.maxcol + 1).platform
        )
    ...
    update: function (dt) {
      if (this.left) this.dx = -this.type.speed;
      else if (this.right) this.dx = this.type.speed;
      else this.dx = 0;

      if (this.up) this.dy = this.type.speed;
      else if (this.down) this.dy = -this.type.speed;
      else this.dy = 0;
  ...
  var Renderer = Class.create({
    initialize: function (images) {
      ...
      this.vscore = 0;
      this.platformWidth =
        2 * tower.or * Math.tan(((360 / tower.cols) * Math.PI) / 360);
    },
    ...
    renderStars: function (ctx) {
      ...
      if (x > 0) ctx.drawImage(this.stars, nx, 0, x, ny, 0, y, x, ny);
      if (y > 0) ctx.drawImage(this.stars, 0, ny, nx, y, x, 0, nx, y);
      if (x > 0 && y > 0) ctx.drawImage(this.stars, nx, ny, x, y, 0, 0, x, y);
    },
    ...
    renderQuadrant: function (ctx, min, max, dir) {
      ...
      while (c != max) {
        for (r = rmin; r <= rmax; r++) {
          ...
          if (cell.platform) this.renderPlatform(ctx, c, y);
          else if (cell.ladder) this.renderLadder(ctx, c, y);
          else if (cell.coin) this.renderCoin(ctx, c, y);
          if (cell.monster) this.renderMonster(ctx, c, y, cell.monster);
        }
    ...
    renderPlatform: function (ctx, col, y) {
      ...
      ctx.fillStyle = Game.Math.darken(
        tower.color.platform,
        60 * Math.min(1, Math.abs(a / 90))
      );
    ...
    renderMonster: function (ctx, col, y, monster) {
      ...
      ctx.drawImage(
        this.images.monster,
        a.x + monster.animationFrame * a.w,
        a.y,
        a.w,
        a.h,
        x,
    ...
    renderPlayer: function (ctx) {
      ctx.drawImage(
        ...
        player.animation.w,
        player.animation.h,
        tx(player.rx, tower.ir) - player.w / 2,
      ...
      if (PLAYER.DEBUG) {
        ...
        ctx.fillRect(
          tx(player.rx, tower.ir) + player.collision.topLeft.x,
          ty(player.ry + player.collision.topLeft.y),
          5,
          5
        );
        ctx.fillRect(
          tx(player.rx, tower.ir) + player.collision.topRight.x,
          ty(player.ry + player.collision.topRight.y),
          -5,
          5
        );
        ctx.fillRect(
          tx(player.rx, tower.ir) + player.collision.middleLeft.x,
          ty(player.ry + player.collision.middleLeft.y),
          5,
          5
        );
        ctx.fillRect(
          tx(player.rx, tower.ir) + player.collision.middleRight.x,
          ty(player.ry + player.collision.middleRight.y),
          -5,
          5
        );
        ctx.fillRect(
          tx(player.rx, tower.ir) + player.collision.bottomLeft.x,
          ty(player.ry + player.collision.bottomLeft.y),
          5,
          -5
        );
        ctx.fillRect(
          tx(player.rx, tower.ir) + player.collision.bottomRight.x,
          ty(player.ry + player.collision.bottomRight.y),
          -5,
          -5
        );
        ctx.fillRect(
          tx(player.rx, tower.ir) + player.collision.ladderUp.x - 2.5,
          ty(player.ry + player.collision.ladderUp.y),
          5,
          5
        );
        ctx.fillRect(
          tx(player.rx, tower.ir) + player.collision.ladderDown.x - 2.5,
          ty(player.ry + player.collision.ladderDown.y),
          5,
          -5
        );
      }
    ...
    createStars: function () {
      return Game.Canvas.render(WIDTH, HEIGHT, function (ctx) {
        var n,
          ...
          sizes = [
            0.25, 0.25, 0.25, 0.25, 0.5, 0.5, 0.5, 0.5, 1, 1, 1, 1, 2, 2,
          ];
        for (n = 0; n < max; n++) {
          ctx.fillStyle = Game.Math.darken(
            Game.Math.randomChoice(colors),
            Game.Math.random(1, 100)
          );
